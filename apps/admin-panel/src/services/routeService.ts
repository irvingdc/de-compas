...
class RouteService {
   ...
   */
  async createSampleRoutes(): Promise<void> {
    try {
      const currentUser = auth.currentUser
      if (!currentUser) {
        throw new Error('Usuario no autenticado')
      ...
      const sampleRoutes: CreateRouteData[] = [
        {
          name: 'CDMX - Guadalajara Express',
          origin: {
          ...
          distance: 460,
          active: true,
          description: 'Ruta express con una parada en Querétaro. Ideal para viajes de negocios.',
          amenities: ['WiFi', 'Aire Acondicionado', 'Asientos Reclinables', 'USB'],
          vehicleType: 'Autobús de Lujo'
        },
        {
          ...
          distance: 520,
          active: true,
          description: 'Ruta cómoda con paradas estratégicas en Aguascalientes y Zacatecas.',
          amenities: ['WiFi', 'Aire Acondicionado', 'Baño'],
          vehicleType: 'Autobús Ejecutivo'
        },
        {
          name: 'CDMX - Cancún Turístico',
          origin: {
            name: 'Terminal TAPO',
            city: 'Ciudad de México',
            state: 'CDMX',
          ...
          distance: 1450,
          active: false,
          description: 'Ruta turística hacia el Caribe mexicano con paradas en ciudades históricas.',
          amenities: ['WiFi', 'Aire Acondicionado', 'Asientos Cama', 'Entretenimiento', 'Baño', 'Snacks'],
          vehicleType: 'Autobús Premium'
        }
      ]

      console.log('Creando rutas de muestra...')
      
      for (const routeData of sampleRoutes) {
        await this.createRoute(routeData)
        console.log(`Ruta creada: ${routeData.name}`)
      }

      console.log('Todas las rutas de muestra han sido creadas exitosamente')
    } catch (error) {
      console.error('Error creando rutas de muestra:', error)
      throw error
    }
  }
